...
class CommentControllerTests extends ControllerTestBase{
    ...
    void testSave() {
		...
		def comment = Comment.get(1)
		assert comment.lastModBy.username == loggedInUser.username
    }
    ...
    void testUpdate() {
        ...
        assert response.redirectedUrl == "/comment/list"
        assert flash.message != null
		assert model.commentInstance.lastModBy.username == loggedInUser.username
	...
	void testListForModuleRedirectWhenNoIdIsGiven() {
		controller.listForModule()
		assert response.redirectedUrl == '/comment/list'
	}
	
	void testListForModule() {
		...
		assert model.commentInstanceList != null
		assert model.commentInstanceList[0].comment == "sample comment #1"
		assert model.commentInstanceList[1].comment == "sample comment #2"
	}
